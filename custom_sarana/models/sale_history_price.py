    ...
    def init(self):
        ...
        self._cr.execute("""
            CREATE VIEW sale_history AS (
                SELECT sol.id as id, sol.order_id, so.date_order, so.partner_id, sol.currency_id,
                    sol.product_id, sol.product_uom_qty, sol.price_unit, sol.discount, sol.price_tax, sol.price_subtotal, sol.price_total, sm.price_unit as cogs_amount
                FROM sale_order so
